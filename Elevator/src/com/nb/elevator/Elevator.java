...
public class Elevator {
	...
	public Comparator<Request> downComparator = new Comparator<Request>() {
		public int compare(Request u1, Request u2) {
			return u2.floor.compareTo(u1.floor);
		}
	};

	private Queue<Request> upQueue = new PriorityQueue<>(100,upComparator);
	private Queue<Request> currentQueue = upQueue;
	private Queue<Request> downQueue = new PriorityQueue<>(100, downComparator);
	...
	private void preProcessNextQueue() {
		if (getLowestTimeUpQueue() > getLowestTimeDownQueue()) {
			...
			upQueue = new PriorityQueue<>(100,upComparator);
		} else {
			this.direction = Direction.DOWN;
			currentQueue = downQueue;
			downQueue = new PriorityQueue<>(100,downComparator);
		}
	}
			...
			try {
				...
				while (true) {
					if ((line = reader.readLine()) != null) {
						System.out.println("Somebody called the lift::" + line);
